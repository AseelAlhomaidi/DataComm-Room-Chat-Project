...
// Function to calculate CRC for a given message
unsigned int calculateCRC(const char *message) {
    unsigned int crc = 0xFFFF; // Initial value

    while (*message) {
        crc ^= (unsigned int)(*message++ << 8);
...
void appendCRC(char *message) {
    unsigned int crc = calculateCRC(message);
    sprintf(message + strlen(message), "%04X", crc); // Assuming a 4-digit hex CRC
}

// Function to check CRC validity
int checkCRC(const char *message) {
    // Extract CRC from the message
    unsigned int receivedCRC;
    if (sscanf(message + strlen(message) - 4, "%X", &receivedCRC) != 1) {
        // Error: unable to extract CRC
        return 0; // CRC check fails
    }

    // Recalculate CRC for the rest of the message (excluding the CRC itself)
    char messageWithoutCRC[256];
    strncpy(messageWithoutCRC, message, strlen(message) - 4);
    messageWithoutCRC[strlen(message) - 4] = '\0';

    if (calculateCRC(messageWithoutCRC) == receivedCRC) {
        // CRC check passes
        return 1;
    } else {
        // CRC check fails
...
// Function to send a message to the server with appended CRC information
void send_with_crc(int clientSocket, char buffer[]) {
    appendCRC(buffer);
        ...
        if (buffer[0] == '@') {
            ...
            for (int j = 0; j < MAX_CLIENTS; j++) {
                if (clients[j].socket != -1 && clients[j].socket != client_socket) {
                    // Send the message to the server with appended CRC
                    send_with_crc(clients[j].socket, pumessage);
                }
            }
        }
    }
